...
export default function (props: VisTimelineContainerProps) {
    ...
    useEffect(() => {
        if (props.entityEvent.status === ValueStatus.Available) {
            const items: DataItem[] = props.entityEvent.items!.map((d, i) => ({
                ...
                content: props.attTitle.get(d).value!,
    ...
    useEffect(() => {
        if (
            props.attStart &&
            props.attEnd &&
            props.attStart.status === ValueStatus.Available &&
            props.attEnd.status === ValueStatus.Available
        ) {
            if (timeLineRef.current) {
    ...
    return <div className={props.class} style={props.style} ref={ref}></div>;
}
